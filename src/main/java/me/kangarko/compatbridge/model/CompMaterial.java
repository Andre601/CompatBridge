...
public enum CompMaterial {
	...
	private CompMaterial(String legacyName, int data) {
		...
		try {
			material = Material.valueOf(MinecraftVersion.atLeast(V.v1_13) ? name() : parseLegacyName(legacyName));
	...
	public static CompMaterial makeMonsterEgg(EntityType type) {
		...
		if (MinecraftVersion.atLeast(V.v1_13) && type == EntityType.PIG_ZOMBIE)
			name = "ZOMBIE_PIGMAN_SPAWN_EGG";

		final CompMaterial mat = fromString(name);
		Objects.requireNonNull(mat, "Could not make entity " + type + "(" + type + "_SPAWN_EGG" + ")");
