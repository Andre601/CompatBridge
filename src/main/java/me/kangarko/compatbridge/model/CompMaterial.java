...
public enum CompMaterial {
	...
	/**
	 * Evaluates whether a given {@link ItemStack} is equal material-wise.
	 * Takes data value in account if we are on Minecraft 1.13 or older.
	 *
	...
	/**
	 * Returns true if the given material is a hard clay (any variations), later named terracotta.
	 *
	 * @param mat
	 * @return
	 */
	...
	public static Material makeMonsterEgg(EntityType type) {
		...
		try {
			...
			mat = Material.valueOf(VersionResolver.isAtLeast1_13() ? "SHEEP_SPAWN_EGG" : "MONSTER_EGG");
		}

		if (mat == null)
			throw new NullPointerException("Could not make entity " + type + "(" + type + "_SPAWN_EGG" + ")");
	...
	public static CompMaterial fromString(String key) {
		try {
			...
			if (split.length == 1)
				xmat = fromLegacy(key, (byte) 0);
			else
				xmat = fromLegacy(split[0], (byte) Integer.parseInt(split[1]));
	...
	/*private static Material getMaterial(String newMaterial, String oldMaterial) {
		try {
	...
	public static Material convertMaterial(Material mat, byte data) {
	return convertMaterial(mat.getId(), data);
	}
